...
pub async fn html_processor(recent_value: &Channel, scrape_type: ScrapeType) -> Option<EmbedData> {
	...
	if let Some(top_url) = html.select(&top_url_selector).next() {
		...
		let (title, img_url, preview_text) = match scrape_type {
			ScrapeType::Forum => {
				(
					post_html.select(&Selector::parse("head>meta:nth-child(5)").unwrap()).next().unwrap().value().attr("content").unwrap_or("").to_string(),
					"".to_string(),
					post_html.select(&Selector::parse("head>meta:nth-child(8)").unwrap()).next().unwrap().value().attr("content").unwrap_or("").to_string()
				)
			}
			ScrapeType::Main => {
				(
					post_html.select(&Selector::parse("head>meta:nth-child(13)").unwrap()).next().unwrap().value().attr("content").unwrap_or("").to_string(),
					{
						let mut actual= "".to_owned();
						let _ = post_html.select(&Selector::parse("img").unwrap()).for_each(|item|{
